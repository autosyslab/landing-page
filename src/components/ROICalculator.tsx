...
function ROICalculator() {
  ...
  const calc = useCallback(() => {
    ...
    calcTimerRef.current = setTimeout(() => {
      ...
      ctaTimerRef.current = setTimeout(() => setCtaVisible(true), 800);
    }, 800);
  }, [monthlyHours, hourlyCost, employees, coverage]);
        ...
        <div className="mt-12 grid lg:grid-cols-2 gap-10">
          ...
          <form onSubmit={(e) => { e.preventDefault(); calc(); }} className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-6 sm:p-8">
            ...
            >
              <input
                id="monthly-hours"
                aria-describedby="monthly-hours-description"
                aria-required="true"
                type="number"
                min={0}
            ...
            >
              <input
                id="hourly-cost"
                aria-describedby="hourly-cost-description"
                aria-required="true"
                type="number"
                min={0}
                className="w-full rounded-xl border-2 border-slate-300 focus:ring-4 focus:ring-cyan-400/50 focus:border-cyan-400 focus:shadow-[0_0_20px_rgba(6,182,212,0.3)] p-3 sm:p-4 text-base sm:text-lg text-slate-800 bg-gradient-to-r from-white to-cyan-50/80 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none hover:bg-gradient-to-r hover:from-cyan-50/80 hover:to-white"
                ...
                <div className="relative bg-gradient-to-br from-white to-purple-50/50 rounded-2xl border border-purple-200/50 p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:border-purple-300">
                  ...
                  <div className="relative mb-4">
                    <input
                      ...
                      className="w-full h-3 rounded-lg appearance-none slider shadow-inner hover:shadow-lg transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-cyan-400/50 cursor-pointer"
                      style={{
            ...
            {isAnimating ? (
              ...
              <div className="w-full space-y-6">
                ...
                <div className={`transform transition-all duration-700 ease-out ${ctaVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`} style={{ transitionDelay: '200ms' }}>
                  <SavingsTicker monthlySavings={out.monthlySavings} isActive={ctaVisible} />
                </div>
...
function EnhancedInput({ icon, label, description, id, children }: {
  ...
  return (
    <div className="mb-6">
      ...
      <p id={`${id}-description`} className="text-sm text-slate-600 mb-3">{description}</p>
      {children}
    </div>
  );
}
      ...
      {isHighlight && (
        <div className="mt-3 h-2 bg-purple-100 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: isVisible ? '100%' : '0%', transitionDelay: `${delay + 300}ms` }}
          />
        </div>
      )}
    </div>
